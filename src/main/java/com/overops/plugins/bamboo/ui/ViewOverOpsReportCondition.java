...
public class ViewOverOpsReportCondition implements Condition {
    ...
    public boolean shouldDisplay(Map<String, Object> context) {
        ...
        if (planKeyDescr == null || buildNumber == null) {
            log.debug("buildNumber or planKeyDescr is null. OverOps report tab will not displaying");
            return false;
        }
        ...
        if (plan == null) {
            log.debug("Plan is null. OverOps report tab will not displaying");
            return false;
        }
        ...
        if (summary == null) {
            log.debug("ResultsSummary is null. OverOps report tab will not displaying");
            return false;
        }

        if (!summary.isInProgress()) { // isFinished() does not include interrupt case
            log.debug("Check is build contains OverOps Step");
            if (summary instanceof ChainResultsSummaryImpl) {
                ...
                for (ChainStageResult chainResult : resultList) {
                    ...
                    for (BuildResultsSummary sum : resultSet) {
                        ...
                        if (customBuildData.containsKey("isOverOpsStep")) {
                            log.debug("Found OverOps step in ChainResultsSummaryImpl");
                            return true;
                        }
                    }
                }
                ...
                if (customBuildData.containsKey("isOverOpsStep")) {
                    log.debug("Found OverOps step in BuildResultsSummaryImpl");
                    return true;
                }
            }

            log.debug("OverOps step not found in current build. OverOps report tab will not displaying");
            return false;
        } else {
            log.debug("Build does not finished yet. OverOps report tab will not displaying");
            return false;
        }
    }
}
